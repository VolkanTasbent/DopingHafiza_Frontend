...
export async function loadWeekdayAvailabilityAsync() {
  try {
    const raw = await AsyncStorage.getItem(WEEKDAY_STORAGE_KEY);
...
export async function saveWeekdayAvailabilityAsync(weekdays) {
  try {
    await AsyncStorage.setItem(WEEKDAY_STORAGE_KEY, JSON.stringify(normalizeAvailableWeekdays(weekdays)));
  } catch {
    /* ignore */
  }
}
